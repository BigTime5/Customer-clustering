...
| Unique customers (after cleaning) | 4,372 |
| Distinct products | 3,684 |
| Total invoices | 22,190 |
| Anonymous rows removed | 135,080 (24.9% of raw data) |
| Duplicate rows removed | 5,225 |
| Invoice-level cancellation rate | 16.47% (3,654 of 22,190) |
| Product segments | 5 |
| Customer segments | 11 |

The dataset records invoices from a UK-based and registered non-store online retailer selling unique all-occasion gifts, with a significant wholesale buyer component. Roughly 65% of all orders exceed £200 in basket value, consistent with a B2B purchase profile.
...
- Rows with missing `CustomerID` values are removed. This accounts for 135,080 rows — roughly 24.9% of the raw dataset — leaving 406,829 rows after this step.
- 5,225 duplicate invoice lines are dropped.
- Cancelled orders (invoice numbers prefixed with `C`) represent 3,654 of 22,190 invoices — a 16.47% cancellation rate. For each cancellation, the pipeline searches for a matching original order. When a match is found, the cancelled quantity is recorded against the original entry and both are removed. When no prior order exists in the dataset window (1,226 cases), the entry is classified as unmatched and discarded. In total, 7,521 matched and 1,226 unmatched cancellation records are removed.
...
**Ensemble architecture.** A `VotingClassifier` with hard voting is constructed. Seven classifiers are trained and evaluated individually before three are selected for the final ensemble based on their generalisation performance: Logistic Regression, K-Nearest Neighbours, and Gradient Boosting.
...
**Evaluation.** Seven classifiers are evaluated individually on the training data before the ensemble is constructed.

| Classifier | Training Precision |
|---|---|
| Support Vector Machine | 85.32% |
| Logistic Regression | 91.14% |
| K-Nearest Neighbours | 78.53% |
| Decision Tree | 82.13% |
| Random Forest | 91.55% |
| AdaBoost | 58.03% |
| Gradient Boosting | 89.34% |
| **Voting Ensemble** | **90.72%** |

The final ensemble is then stress-tested on the hold-out test set of unseen customers, where ground-truth labels are generated by applying the trained K-Means model to the scaled test features. Performance on unseen data is as follows.

| Classifier | Test Precision |
|---|---|
| Support Vector Machine | 69.74% |
| Logistic Regression | 75.11% |
| K-Nearest Neighbours | 67.11% |
| Decision Tree | 70.60% |
| Random Forest | 74.72% |
| Gradient Boosting | 73.15% |
| **Voting Ensemble** | **75.07%** |

The ensemble's test precision of **75.07%** means that in three out of four cases, a customer's archetype predicted from a single basket matches the segment they would be assigned based on their full purchase history. At inference time, the API also returns individual model votes and confidence scores for each base learner, giving the business user visibility into prediction agreement across the ensemble.

---

## 4. Segment Profiles

The eleven segments span a wide range of behavioural patterns. Customer counts below reflect the training window population of 4,372 customers.
